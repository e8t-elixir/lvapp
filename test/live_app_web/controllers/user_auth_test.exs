defmodule LiveAppWeb.UserAuthTest do
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, LiveAppWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "log_in_user/3" do
    test "stores the user token in the session", %{conn: conn, user: user} do
      ...
      assert PGA.get_user_by_session_token(token)
    end
  ...
  describe "logout_user/1" do
    test "erases session and cookies", %{conn: conn, user: user} do
      user_token = PGA.generate_user_session_token(user)
      ...
      assert redirected_to(conn) == "/"
      refute PGA.get_user_by_session_token(user_token)
    end

    test "broadcasts to the given live_socket_id", %{conn: conn} do
      live_socket_id = "users_sessions:abcdef-token"
      LiveAppWeb.Endpoint.subscribe(live_socket_id)
